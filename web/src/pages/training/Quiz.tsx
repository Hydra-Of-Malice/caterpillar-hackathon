...
import { Button, EmptyState, ErrorNote, Icon, Loading, PageTitle, Panel, PanelHeader, ProgressBar, cx } from '../../components/ui';
...
function TruckLoadingScene({ showDistance }: { showDistance: boolean }) {
  return (
    <svg viewBox="0 0 520 240" className="h-full w-full" role="img" aria-label="Top-down sketch: excavator swinging a loaded bucket from the dig face towards the truck body; the truck cab is a no-swing zone">
      ...
      <text x="477" y="140" textAnchor="middle" fill="#FF6B66" fontSize="11" fontFamily="Roboto Condensed, sans-serif" fontWeight="700">CAB · NO SWING</text>
      ...
      <rect x="290" y="90" width="18" height="18" fill="var(--svg-text)" stroke="#0E0E0E" />
      {/* swing path */}
      <path d="M 248 206 Q 262 140 286 108" fill="none" stroke="#FB5A00" strokeWidth="3" strokeDasharray="7 5" markerEnd="url(#qz-arrow)" />
      <text x="206" y="162" fill="#FF9A40" fontSize="12" fontFamily="Roboto Condensed, sans-serif" fontWeight="700">SWING PATH</text>
      {showDistance && (
        <g>
...
function TrenchScene() {
  return (
    <svg viewBox="0 0 520 240" className="h-full w-full" role="img" aria-label="Top-down sketch: excavator behind the setback line beside a trench, with the spoil pile placed back from the edge">
      <rect x="0" y="0" width="520" height="240" fill="var(--chart-tip-bg)" />
      <rect x="40" y="150" width="440" height="46" fill="#080808" stroke="var(--chart-tip-border)" strokeWidth="2" />
      ...
      <rect x="220" y="158" width="18" height="18" fill="var(--svg-text)" stroke="#0E0E0E" />
    </svg>
  );
}
...
function OptionCard({
  ...
  return (
    <button
      ...
      onClick={onSelect}
      className={cx(
        'flex min-h-[76px] w-full items-center gap-4 border-2 px-4 py-3 text-left transition-colors duration-quick disabled:cursor-default',
        isCorrect && 'border-success bg-success/15',
        isWrong && 'border-danger bg-danger/15',
        !isCorrect && !isWrong && selected && 'border-cat bg-cat/10',
        !isCorrect && !isWrong && !selected && (locked ? 'border-outline bg-surface-container-low opacity-60' : 'border-outline bg-surface-container-low hover:border-outline-strong hover:bg-surface-container-high'),
      )}
    >
      <span
        className={cx(
          'flex h-10 w-10 shrink-0 items-center justify-center border-2 font-display text-headline-sm',
          isCorrect ? 'border-success-text text-success-text' : isWrong ? 'border-danger-text text-danger-text' : selected ? 'border-cat text-cat-text' : 'border-outline-strong text-on-surface-variant',
        )}
      >
        {isCorrect ? <Icon name="check" size={24} /> : isWrong ? <Icon name="close" size={24} /> : letter}
      </span>
      <span className="flex-1 text-body-lg text-on-surface">{text}</span>
      {isCorrect && <span className="font-display text-label-sm uppercase text-success-text">Safer answer</span>}
      {isWrong && <span className="font-display text-label-sm uppercase text-danger-text">Your answer</span>}
    </button>
  );
}
...
function Feedback({ q, chosen }: { q: QuizQuestion; chosen: string }) {
  ...
  if (!known) {
    return (
      <div className="flex items-start gap-3 border-2 border-notice-dark bg-notice/10 p-4" role="status">
        <Icon name="task_alt" size={28} className="text-notice-dark" />
        <div className="space-y-2">
          <div className="font-display text-label-lg uppercase text-on-surface">Answer recorded</div>
  ...
  return (
    <div className={cx('flex items-start gap-3 border-2 p-4', correct ? 'border-success bg-success/10' : 'border-danger bg-danger/10')} role="status">
      <Icon name={correct ? 'check_circle' : 'cancel'} size={28} fill className={correct ? 'text-success-text' : 'text-danger-text'} />
      <div className="min-w-0 flex-1 space-y-2">
        <div className={cx('font-display text-label-lg uppercase', correct ? 'text-success-text' : 'text-danger-text')}>{correct ? 'Correct' : 'Not quite'}</div>
        {!correct && right && (
...
export default function Quiz() {
  ...
  const header = (
    <>
      <TrainingTabs />
      <PageTitle
        kicker="Scenario quiz · knowledge check"
        title={`Scenario quiz — ${title}`}
        sub={questions.length ? `Pass mark ${quiz?.pass_mark ?? Math.max(1, questions.length - 1)} of ${questions.length}` : undefined}
        right={
          <>
            <ProvenanceBadge kind="RULE" />
            <DataSourceChip endpoints={['/training/quiz/']} />
          </>
        }
      />
    </>
    ...
    return (
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 gap-6 xl:grid-cols-3">
          <Panel accent={result.passed ? 'green' : 'orange'} className="space-y-5 p-6 pl-7 xl:col-span-2">
            <div className="flex flex-wrap items-center gap-4">
              <Icon name={result.passed ? 'emoji_events' : 'replay'} size={48} className={result.passed ? 'text-success-text' : 'text-warning-text'} />
              <div>
                <div className="font-display text-label-sm uppercase text-on-surface-muted">Result</div>
                <div className="font-display text-headline-xl text-on-surface">
                  Score <span className="tnum">{result.score}/{result.total}</span> — {result.passed ? 'passed' : 'not passed yet'}.
                </div>
              </div>
            </div>
            {result.passed ? (
              <div className="space-y-1 text-body-lg text-on-surface-variant">
                <p>Next: practise on the next shift.</p>
                <p>{result.next ?? `Your ${competencyLabel(competencyId).toLowerCase()} will be checked automatically over the next 3 shifts.`}</p>
              </div>
            ) : (
              <p className="text-body-lg text-on-surface-variant">Review the key points and try again. You need {quiz?.pass_mark ?? result.total - 1} of {result.total} to pass.</p>
            )}
            <div className="flex flex-wrap items-center gap-3">
              {result.passed ? (
                <>
                  <span className="inline-flex items-center gap-2">
                    <Button variant="primary" size="lg" icon="sports_esports" onClick={() => navigate(`/training/booking?topic=${encodeURIComponent(competencyId)}&format=simulator`)}>
                      Book simulator practice
                    </Button>
                    <ProvenanceBadge kind="MOCK" />
                  </span>
                  <Button variant="secondary" size="lg" icon="done" onClick={() => navigate('/training')}>
                    Done
                  </Button>
                </>
              ) : (
                <>
                  <Button variant="primary" size="lg" icon="replay" onClick={reset}>
                    Try again
                  </Button>
                  <Button variant="secondary" size="lg" icon="menu_book" onClick={() => navigate(`/training/module/${encodeURIComponent(id)}`)}>
                    Review module
                  </Button>
                  <Button variant="secondary" size="lg" icon="done" onClick={() => navigate('/training')}>
                    Done
            ...
            <div className="flex items-start gap-2 border border-outline-variant bg-surface-container-low px-4 py-3">
              <Icon name="verified_user" size={20} className="mt-0.5 text-notice-dark" />
              <p className="text-body-sm text-on-surface-variant">
                This quiz is a knowledge check. A quiz pass never sets DEMONSTRATED for safety-critical competencies — an instructor verifies on the machine or simulator.{' '}
                {competencyLabel(competencyId)} stays below DEMONSTRATED until then.
              </p>
            </div>
            <Link to="/training/effect" className="inline-flex items-center gap-1 font-display text-label-md uppercase text-notice-dark hover:underline">
              See whether the training helped <Icon name="arrow_forward" size={18} />
            </Link>
          </Panel>

          <Panel>
            <PanelHeader icon="fact_check" title="Your answers" />
                ...
                return (
                  <li key={q.question_id} className="flex items-start gap-3 px-4 py-3">
                    <Icon
                      ...
                      size={22}
                      className={ok === null ? 'text-notice-dark' : ok ? 'text-success-text' : 'text-danger-text'}
                      title={ok === null ? 'Recorded' : ok ? 'Correct' : 'Incorrect'}
                    />
                    <div className="min-w-0">
                      <div className="font-display text-label-sm uppercase text-on-surface-muted">Question {i + 1}</div>
                      <p className="text-body-sm text-on-surface-variant">{q.prompt}</p>
                    </div>
                  </li>
                );
              })}
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-6 xl:grid-cols-3">
        <Panel className="xl:col-span-2">
          <div className="space-y-3 border-b border-outline px-5 py-4">
            <div className="flex flex-wrap items-center justify-between gap-2">
              <span className="font-display text-label-md uppercase text-on-surface">
                Question <span className="tnum">{idx + 1}</span> of <span className="tnum">{questions.length}</span>
              </span>
                ...
                {questions.map((qq, i) => {
                  ...
                  return (
                    <span
                      ...
                      className={cx(
                        'flex h-6 w-6 items-center justify-center border font-display text-[11px] font-bold',
                        i === idx ? 'border-cat text-cat-text' : 'border-outline-variant text-on-surface-muted',
                        c && ok === true && 'border-success bg-success/20 text-success-text',
                        c && ok === false && 'border-danger bg-danger/20 text-danger-text',
                      )}
                    >
                      {c && ok === true ? <Icon name="check" size={14} /> : c && ok === false ? <Icon name="close" size={14} /> : i + 1}
                    </span>
                  );
                })}
              </div>
            </div>
            <ProgressBar pct={(answeredCount / questions.length) * 100} tone="yellow" height="h-2.5" />
          </div>

          <div className="space-y-5 p-5">
            <div className="relative aspect-[13/6] w-full overflow-hidden border border-outline">
              {trench ? <TrenchScene /> : <TruckLoadingScene showDistance={/4\s?m\b/.test(q.prompt)} />}
              <span className="absolute bottom-2 left-2 font-display text-[11px] uppercase tracking-[0.06em] text-on-surface-muted">Top-down illustration · not to scale</span>
            </div>
            <h2 className="font-display text-headline-md text-on-surface">{q.prompt}</h2>
            ...
            <div className="flex flex-wrap items-center justify-between gap-3 border-t border-outline pt-4">
              <Button variant="secondary" icon="arrow_back" disabled={idx === 0} onClick={() => setIdx((i) => Math.max(0, i - 1))}>
                Previous
              </Button>
              {!isChecked ? (
                <Button variant="primary" size="lg" icon="check" disabled={!chosen} onClick={() => setChecked((c) => ({ ...c, [q.question_id]: true }))}>
                  Check answer
                </Button>
              ) : last ? (
                <Button variant="primary" size="lg" icon="flag" disabled={submitting} onClick={finish}>
                  {submitting ? 'Submitting…' : 'Finish quiz'}
                </Button>
              ) : (
                <Button variant="primary" size="lg" iconRight="arrow_forward" onClick={() => setIdx((i) => Math.min(questions.length - 1, i + 1))}>
                  Next question
                </Button>
              )}
            </div>
        ...
        <aside className="space-y-4">
          <Panel className="space-y-3 p-4">
            <div className="flex items-center gap-2 font-display text-label-md uppercase text-on-surface">
              <Icon name="info" size={20} className="text-notice-dark" /> How this quiz works
            </div>
          ...
          <Panel className="space-y-2 p-4">
            <div className="font-display text-label-sm uppercase text-on-surface-muted">Competency</div>
            <div className="font-display text-label-lg uppercase text-on-surface">{competencyLabel(competencyId)}</div>
            <Link to={`/training/module/${encodeURIComponent(id)}`} className="inline-flex items-center gap-1 font-display text-label-sm uppercase text-notice-dark hover:underline">
              <Icon name="arrow_back" size={16} /> Back to module
            </Link>
          </Panel>
        </aside>
      </div>
